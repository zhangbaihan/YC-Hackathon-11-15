...
4. Serve the API with `uv run uvicorn app.main:app --reload --host 0.0.0.0 --port 8000` or regenerate the static artifact with `uv run python scripts/generate_commerce.py`.
...
- `GET /commerce.txt` — Serves the cached markdown artifact generated from the latest J.Crew product snapshot.
...
- `scripts/generate_commerce.py` — CLI helper (wired to `uv run commerce-txt`) to rebuild the artifact.
...
- `data/commerce.txt` — The generated markdown served as `/commerce.txt`.
...
- `uv run python scripts/generate_commerce.py -- --limit 10` — regenerate `data/commerce.txt` from the saved J.Crew page (pass script args after `--`).
...
markdown = pipeline.write_markdown(
    ...
    Path("data/commerce.txt"),
    title="J.Crew men sweaters",
)
```

3. Feed the resulting `products` list (via `JCrewPlpParser`) into `MarkdownCompressor` or any other downstream consumer, or simply serve the generated `data/commerce.txt` through the API.

Run `pytest tests/test_jcrew_parser.py tests/test_pipeline.py` to ensure the extractor and pipeline continue to match the upstream payload shape.
